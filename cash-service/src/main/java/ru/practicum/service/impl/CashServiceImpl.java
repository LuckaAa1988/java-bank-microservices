...
public class CashServiceImpl implements CashService {
    ...
    public Mono<Void> deposit(AccountDto accountDto) {
        return blockerClient.checkDeposit(accountDto)
                ...
                .then(notificationClient.sendSuccessNotification(NotificationDto.builder()
                        ...
                        .data(String.format("%s на счёт %s успешно зачислены!", accountDto.getAmount(), accountDto.getCurrency()))
                        .build()))
                .onErrorResume(error -> notificationClient.sendFailureNotification(NotificationDto.builder()
                                .username(accountDto.getUsername())
                                .data(String.format("Зачисление %s на счёт %s заблокировано!", accountDto.getAmount(), accountDto.getCurrency()))
    ...
    public Mono<Void> withdraw(AccountDto accountDto) {
        return blockerClient.checkWithdraw(accountDto)
                ...
                .then(notificationClient.sendSuccessNotification(NotificationDto.builder()
                        .username(accountDto.getUsername())
                        .data(String.format("%s на счёт %s успешно сняты!", accountDto.getAmount(), accountDto.getCurrency()))
                        .build()))
                .onErrorResume(error -> notificationClient.sendFailureNotification(NotificationDto.builder()
                                .username(accountDto.getUsername())
                                .data(String.format("Снятие %s со счёта %s заблокировано!", accountDto.getAmount(), accountDto.getCurrency()))
                                .exception(error.getMessage())
                                .build())
                        .then(Mono.error(error)));
    }
